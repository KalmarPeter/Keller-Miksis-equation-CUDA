...
__global__ void OdeSolver(double*, double*, double*, double*);
...
int main()
{
	...
	double pa1[] = { 1.0e5 }; // pressure amplitude 1
	double pa2[] = { 0.5e5 }; // pressure amplitude 2
	...
	Samplog(LowerBoundary, UpperBoundary, Resolution, fr1);
	Samplog(LowerBoundary, UpperBoundary, Resolution, fr2);

	// Copy the exacition parameters to the device

	for (int i = 0; i < Resolution; i++)
	{
		omega1[i] = 2.0 * M_PI * fr1[i];
		omega2[i] = 2.0 * M_PI * fr2[i];
	}
	...
	{
		OdeSolver << <GridSize, BlockSize >> > (d_t0, d_y, d_c, d_Rm);
...
__global__ void GetC(double* d_omega1, double* d_omega2, double* d_c, double* d_penv, double* d_pv, double* d_R0, double* d_rho, double* d_cl, double* d_n, double* d_sft, double* d_pa1, double* d_pa2, double* d_nu, double* d_theta)
{
	...
	d_c[tid + Resolution] = (((1.0 - 3.0 * r_n) * 2.0 * M_PI) / (r_R0 * d_omega1[tid] * r_rho * r_cl)) * (((2.0 * r_sft) / r_R0) + r_penv - r_pv);
...
__forceinline__ __device__ void rkck(double* t0, double* y, double* c, double& dt, double* yn, double* error)
{
	...
	for (int i = 0; i < SysDim; ++i)
	{
		error[tid + i * Resolution] = fabs(((37.0 / 378.0) - (2825.0 / 27648.0)) * k1[i] + (0.0) * k2[i] + ((250.0 / 621.0) - (18575.0 / 48384.0)) * k3[i] + ((125.0 / 594.0) - (13525.0 / 55296.0)) * k4[i] + ((0.0) - (277.0 / 14336.0)) * k5[i] + ((512.0 / 1771.0) - (1.0 / 4.0)) * k6[i]) + 1.0e-18;
	}
...
__global__ void OdeSolver(double* d_t0, double* d_y, double* c, double* d_Rm)
{
